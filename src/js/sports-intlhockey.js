(function () {
  ...
  // International hockey has no continental championships, so all four global
  // events live in one row. 👑 marks the global pinnacle (Olympics); 🏆 for
  // the rest (handled per-tournament in renderLeagueHistory). FORSBERG's own
  // source has a duplicate 'Olympics' entry here (copy-paste leftover,
  // verified live) - collapsed to the single correct entry.
  var HISTORY_WORLD = [
    { value: "Olympics", label: "👑 Olympics" },
    { value: "World Cup of Hockey", label: "🏆 World Cup of Hockey" },
    { value: "Canada Cup", label: "🏆 Canada Cup" },
  ...
  var state = {
    ...
    tsConf: "all",
    tsView: "cross",
    historyTourny: "Olympics",
    championsData: null,
    goatData: null,
    goatConf: "all",
  };

  // ── formatting helpers, ported 1:1 from FORSBERG's docs/index.html ────────
  ...
  // Reuses ICHIRO's badge-ib-* namespaced classes - FORSBERG's own confederation
  // colors are an exact hex match (Asia #cc0000, Americas #0055a4, Europe
  // #f5c518, Africa #00843d, Oceania #7b1fa2), so no new namespace is needed.
  ...
  // 👑 reserved for Olympic gold (the global pinnacle); 🏆 for World Cup of
  // Hockey / Canada Cup / IIHF World Championship gold.
  function finishBadge(finishes) {
    ...
    function medal(x) {
      if (x.finish === 1) return x.tournament === "Oly" ? "👑" : "🏆";
  ...
  // Hockey has ties, unlike the other international-fleet sports built so far.
  function resultClass(match, isStale) {
    ...
    if (first === "T") return "result-T" + suffix;
    return "";
  }
  ...
  var LAST_MATCH_RE = /^([WLT])\s+(\d+\s*-\s*\d+)\s+(vs\.?(?:\s*\(N\))?|@)\s+(.+?)\s*(\([^)]+\))?\s*$/;

  // FORSBERG format: "W 5-2 vs. Sweden (IIHF World Championship)" - full
  // tournament tag preserved. Opponent country names match teamsIndex; they
  // become deep-links.
  ...
  function seasonLinkClick(season, targetConf) {
    ...
    if (targetConf && STANDINGS_CONFS[targetConf]) {
      ...
      buildConfPills("fbConfPills", true, state.standingsConf, function (v) { state.standingsConf = v; renderStandings(); });
    }
  ...
  // International hockey events are all global (multi-confederation), so a
  ...
  document.getElementById("fbTabs").addEventListener("click", function (e) {
  ...
  var seasonSelect = document.getElementById("fbSeason");
  var dateSelect = document.getElementById("fbDate");
  var countEl = document.getElementById("fbCount");
  var standingsTableWrap = document.getElementById("fbStandingsTable");
  var dateRangeEl = document.getElementById("fbDateRange");
  ...
  function renderStandings() {
    ...
    var rows = teams.map(function (t) {
      ...
      var cellInner = '<span class="sport-flag">' + (t.flag || "") + "</span>" + (t.display_name || t.team);
      var teamTd = slug
    ...
    standingsTableWrap.innerHTML =
      ...
      '<th>Country</th><th>Conf</th><th class="col-record">W-T-L</th><th>Rating</th>' +
  ...
  // Olympic finish medal on its own line atop the record cell: 👑 gold,
  // 🥈 silver, 🥉 bronze. Mirrors finishBadge's glyphs.
  function olyMedal(finishes) {
    var o = (finishes || []).filter(function (x) { return x.tournament === "Oly"; })[0];
    if (!o) return "";
    return o.finish === 1 ? "👑" : o.finish === 2 ? "🥈" : o.finish === 3 ? "🥉" : "";
  }

  // Olympic edition record cell: finish medal on its own line (omitted with no
  // medal / in progress), then the edition W-T-L. Combined record (no
  // group/knockout split - hockey has no reliable stage data); hockey games
  // can end in a tie, unlike basketball/baseball.
  function fmtOlyRecord(r, finishes) {
    if (!r) return "0-0-0";
    var e = olyMedal(finishes);
    var medalLine = e ? '<div style="line-height:1.3">' + e + "</div>" : "";
    return medalLine + r.w + "-" + r.t + "-" + r.l;
  }

  // Olympics view: the selected country's OWN rank·rating walk across the
  // edition, rendered as an offset "stairstep" beside the game list. The walk
  // has N+1 boundary nodes (pre-tournament .. post-final) for N games. Both
  // columns use fixed WC_ROW-tall rows; the games cell is pushed down half a
  // row (wcMatchesCell) so each game lands in the GAP between the two walk
  // nodes it sits between - the node below a game is the rating going IN, the
  ...
  // drop (mirrors the season chart's +D/-D palette); the arrow shows rank
  // movement.
  ...
  function wcWalkCell(walk) {
    var nodes = (walk || []).slice().reverse(); // top = end of tournament, bottom = pre-tournament
  ...
  function wcMatchesCell(g) {
    var ms = ((g.oly_record && g.oly_record.matches) || []).slice().reverse();
    var html = ms.map(function (mm) {
      ...
      return '<div style="height:' + WC_ROW + 'px;display:flex;align-items:center;white-space:nowrap">' + date +
        "<span>" + renderLastMatch(mm.s, g.season, false, "olympics") + opp + "</span></div>";
    }).join("");
  ...
  function renderTeamTable() {
    ...
    if (state.tsView === "single") {
      ...
      seasons.forEach(function (s) {
        data.seasons[s].forEach(function (g) {
          if (g.oly_record) rows.push(Object.assign({}, g, { season: s }));
        });
      });
      if (rows.length === 0) {
        tsChartWrap.hidden = true;
        tsTableWrap.innerHTML = '<p class="sport-loading">No Olympic hockey appearances in the rating era</p>';
        return;
      }
    } else {
      seasonFilter = "all";
    ...
    var isOly = state.tsView === "olympics";
    ...
    tsTableWrap.innerHTML =
      '<table class="sport-table"><thead><tr>' +
      '<th style="text-align:center">Year</th>' + (isOly ? "" : '<th class="col-hide-mobile">Date</th>') +
      (isOly ? '<th>Rating Evolution<div class="sub-line">Rank · Rating</div></th>' : "") +
      "<th>" + (isOly ? 'Games<div class="sub-line">opponent rank · rating prior to game</div>' : "Last Game") + "</th>" +
      (isOly ? "<th>W-T-L</th>" : "") +
      '<th class="col-rank">OVR #</th><th class="col-hide-mobile col-rank">Conf #</th>' +
      "<th>" + (isOly ? "Final Rating" : "Rating") + "</th>" +
      '<th class="col-hide-mobile col-conf">Conf</th>' +
      (isOly ? "" : '<th class="col-hide-mobile">Honors</th>') +
      "</tr></thead><tbody>" + tableRows + "</tbody></table>";
    attachLinks(tsTableWrap);
  }

  // Hand-rolled SVG rating-over-time chart, ported from FORSBERG's drawChart().
  // No trophy markers - the Olympics view's own rank/rating walk already
  ...
  function renderLeagueHistory(league) {
    ...
    // 👑 reserved for Olympic gold counts (the global pinnacle); 🏆 elsewhere.
    var champEmoji = league === "Olympics" ? "👑" : "🏆";
    ...
    function teamCell(t, bg, season, preRated) {
      ...
      var cellInner = '<span class="sport-flag">' + (t.flag || "") + "</span>" + (t.display_name || t.team) + countStr + wlStr;
      if (preRated) {
        return (
    ...
    // International hockey events are all global, so a year-click never
    ...
    var footnote = anyPreRated
      ? '<p class="sport-note"><sup>†</sup> Editions before FORSBERG ratings begin (or with incomplete game data). Podium shown for completeness; ratings are not available at these editions\' final dates.</p>'
      : "";
