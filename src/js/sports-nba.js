(function () {
  ...
  // ── formatting helpers, ported 1:1 from DUNCAN's docs/index.html ──────────
  ...
  function confBadge(conf, finalsStatus) {
    if (!conf) return "";
    if (finalsStatus >= 1) {
  ...
  function finishBadge(finalsStatus, cupStatus) {
    ...
    else if (finalsStatus === 1) bits.push('<span class="finish-emoji" title="NBA Runner-Up">🥈</span>');
    if (cupStatus === 2) bits.push('<span class="finish-badge finish-champion" title="NBA Cup Champion">Cup 🏆</span>');
    else if (cupStatus === 1) bits.push('<span class="finish-badge finish-runner" title="NBA Cup Runner-Up">Cup 🥈</span>');
    return bits.join(" ");
  }
  ...
  // NBA Cup final: a real played game that moves the rating but does NOT
  // count in the W-L record. Tagged so that's self-evident.
  function renderLastMatch(raw, season, isStale, isCup) {
    var display = displayMatch(raw);
    if (!display) return "-";
    var cupTag = isCup
      ? ' <span class="cup-final-tag" title="NBA Cup final (counts for ratings, not the W-L record)">Cup Final</span>'
      : "";
  ...
  document.getElementById("nbaTabs").addEventListener("click", function (e) {
  ...
  var seasonSelect = document.getElementById("nbaSeason");
  var dateSelect = document.getElementById("nbaDate");
  var countEl = document.getElementById("nbaCount");
  var warmupNote = document.getElementById("nbaWarmup");
  var standingsTableWrap = document.getElementById("nbaStandingsTable");
  var dateRangeEl = document.getElementById("nbaDateRange");
  var refreshedEl = document.getElementById("nbaRefreshed");

  var titleOddsTitle = "Probability of winning the NBA Finals, from simulating the rest of the regular season and the playoffs with current ratings. League-wide probabilities sum to 100%.";
  ...
  function renderTeamTable() {
    ...
    var tableRows = rows.slice().reverse().map(function (g) {
      ...
      var seasonCellRaw = isSingle ? fmtSeason(g.season) : fmtSeason(g.season) + seasonTag(g.season);
      var seasonCell = era ? seasonCellRaw + '<div class="sub-line-italic">' + era + "</div>" : seasonCellRaw;
      ...
      return (
        ...
        '<td class="col-hide-mobile col-conf">' + confBadge(g.conference || data.conference, g.finals_status) + "</td>" +
  ...
  // Hand-rolled SVG rating-over-time chart, ported from DUNCAN's drawChart().
  function drawChart(rows, seasonFilter) {
    ...
    rows.forEach(function (r, i) {
      ...
      var titleText = (r.finals_status === 2 ? "NBA Champion (" : "NBA Runner-Up (") + r.season + ")";
      var x = px(i);
  ...
  function cupInlineBadge(t) {
    if (t.cup_status === 2) return ' <span class="finish-badge finish-champion" title="NBA Cup Champion">Cup 🏆</span>';
    if (t.cup_status === 1) return ' <span class="finish-badge finish-runner" title="NBA Cup Runner-Up">Cup 🥈</span>';
    return "";
  }
  ...
  function championTeamCell(t, bg, season) {
    ...
    var nameHtml = label + countStr(t) + cupInlineBadge(t);
    var teamTd = slug
  ...
  function renderChampions() {
    ...
    var rows = entries.map(function (e) {
      return (
        "<tr>" +
        '<td class="season-cell linked" data-season-link="' + e.season + '">' + fmtSeason(e.season) + seasonTag(e.season) + "</td>" +
  ...
  function rankSeasonCell(e) {
    return '<td class="season-cell linked" data-season-link="' + e.season + '">' + fmtSeason(e.season) + seasonTag(e.season) + "</td>";
  }
  ...
  function renderRankings(rankType) {
    ...
    var entries = (state.championsData.NBA || []).filter(function (e) {
    ...
    switch (rankType) {
      case "best-champs":
        ...
        note = "Top 10 champions by end-of-playoffs rating. The strongest teams ever to win an NBA Finals.";
        mode = "team-vs-opp"; break;
      case "worst-champs":
        sorted = rows.slice().sort(function (a, b) { return a.champ.rating - b.champ.rating; }).slice(0, 10);
        note = "Bottom 10 champions by end-of-playoffs rating. The weakest teams ever to win an NBA Finals.";
        mode = "team-vs-opp"; break;
      case "best-losers":
        sorted = rows.slice().sort(function (a, b) { return b.loser.rating - a.loser.rating; }).slice(0, 10);
        note = "Top 10 runner-ups by end-of-playoffs rating. The strongest teams ever to lose an NBA Finals.";
        mode = "loser-vs-champ"; break;
      case "worst-losers":
        sorted = rows.slice().sort(function (a, b) { return a.loser.rating - b.loser.rating; }).slice(0, 10);
        note = "Bottom 10 runner-ups by end-of-playoffs rating. The weakest teams ever to lose an NBA Finals.";
        mode = "loser-vs-champ"; break;
      case "best-off-champs":
        sorted = rows.slice().sort(function (a, b) { return b.champ.rating - a.champ.rating; }).slice(0, 10);
        note = "Top 10 champions by Offense rating. The most explosive offenses ever to win an NBA Finals.";
        mode = "team-vs-opp"; break;
      ...
      case "best-def-champs":
        sorted = rows.slice().sort(function (a, b) { return b.champ.rating - a.champ.rating; }).slice(0, 10);
        note = "Top 10 champions by Defense rating. The most suffocating defenses ever to win an NBA Finals.";
        mode = "team-vs-opp"; break;
      ...
      case "upset":
        ...
        note = sorted.length + " NBA Finals where the model picked the loser by pre-Finals rating. Sorted from biggest upset down.";
        mode = "spread"; break;
      default:
  ...
  function renderRepeatChamps() {
    var entries = (state.championsData.NBA || []).slice().sort(function (a, b) { return a.season - b.season; });
    var runs = [];
    var current = [];
  ...
  function renderRematches() {
    ...
    historyRankNote.textContent = groups.length + " NBA Finals rematch groups (" + totalMeetings + " total meetings). A group continues as long as the same two teams meet again within " + REMATCH_GAP_THRESHOLD + " years.";
    if (!groups.length) {
  ...
  function renderBiggestLeap() {
    var rows = (state.championsData.NBA || [])
    ...
    var body = rows.map(function (r, i) {
      ...
      var nameHtml = label + countStr(c) + cupInlineBadge(c);
      var teamTd = slug
  ...
  function renderGoat() {
    ...
    var rows = teams.map(function (t) {
      ...
      return (
        ...
        '<td class="col-rank linked" data-season-link="' + t.season + '">' + fmtSeason(t.season) + seasonTag(t.season) + "</td>" +
        teamTd +
        '<td class="col-hide-mobile col-conf">' + confBadge(t.conference, t.finals_status) + "</td>" +
  ...
  // ═══════════════════════════════ NBA Playoffs ═══════════════════════════
  // Knockout odds, ported from MESSI's World Cup grid (same code as sports-wnba.js). Data: per-season
  ...
  var poNav = document.getElementById("nbaPoNav");
  ...
  function renderPlayoffOdds() {
    ...
    var nR = short.length;
    poTitle.textContent = d.season + " NBA Playoffs 🏆 Win Probability";
  ...
  Promise.all([
    ...
    buildPills("nbaConfPills", state.standingsConf, function (v) { state.standingsConf = v; renderStandings(); }, [
      { value: "ALL", label: "All" }, { value: "East", label: "East" }, { value: "West", label: "West" },
    ]);
