(function () {
  ...
  // Tournaments-tab pill groups. 👑 marks the global pinnacle (Olympics);
  // 🏆 for everything else (handled per-tournament in renderLeagueHistory).
  var HISTORY_WORLD = [
    { value: "Olympics", label: "👑 Olympics" },
    { value: "FIBA World Cup", label: "🏆 FIBA World Cup" },
  ];
  var HISTORY_CONTINENTAL = [
    { value: "EuroBasket", label: "🇪🇺 EuroBasket" },
    { value: "FIBA AmeriCup", label: "🌎 FIBA AmeriCup" },
    { value: "FIBA Asia Cup", label: "🌏 FIBA Asia Cup" },
    { value: "FIBA AfroBasket", label: "🌍 FIBA AfroBasket" },
  ];
  ...
  // ── formatting helpers, ported 1:1 from CARMELO's docs/index.html ─────────
  ...
  // 👑 reserved for Olympic gold (THE singular global championship for
  // basketball); 🏆 for World Cup + continental tournament gold.
  ...
  // CARMELO format: "W 88-83 vs. Turkey (EuroBasket)" - full tournament tag
  ...
  function seasonLinkClick(season, targetConf) {
    ...
    if (targetConf && STANDINGS_CONFS[targetConf]) {
      ...
      buildConfPills("cmConfPills", true, state.standingsConf, function (v) { state.standingsConf = v; renderStandings(); });
    }
  ...
  // Returns empty for global tournaments (Olympics / World Cup - multi-confed).
  function tournamentConf(t) {
    if (!t) return "";
    if (t === "EuroBasket") return "Europe";
    if (t === "FIBA AmeriCup") return "Americas";
    if (t === "FIBA Asia Cup") return "Asia/Oceania";
    if (t === "FIBA AfroBasket") return "Africa";
    return "";
  }
  ...
  var seasonSelect = document.getElementById("cmSeason");
  var dateSelect = document.getElementById("cmDate");
  var countEl = document.getElementById("cmCount");
  var standingsTableWrap = document.getElementById("cmStandingsTable");
  var dateRangeEl = document.getElementById("cmDateRange");
  ...
  // medal / in progress), then the edition W-L. Combined record (no group/
  // knockout split - basketball has no reliable stage data).
  function fmtOlyRecord(r, finishes) {
    if (!r) return "0-0";
    var e = olyMedal(finishes);
    var medalLine = e ? '<div style="line-height:1.3">' + e + "</div>" : "";
    return medalLine + r.w + "-" + r.l;
  }
  ...
  function renderTeamTable() {
    ...
    if (state.tsView === "single") {
      ...
      if (rows.length === 0) {
        tsChartWrap.hidden = true;
        tsTableWrap.innerHTML = '<p class="sport-loading">No Olympic basketball appearances in the rating era</p>';
        return;
      }
    } else {
      seasonFilter = "all";
  ...
  // Hand-rolled SVG rating-over-time chart, ported from CARMELO's drawChart().
  // No trophy markers (unlike DUNCAN/LOBO) - the Olympics view's own
  // rank/rating walk already carries that context.
  ...
  function renderLeagueHistory(league) {
    ...
    // Year-cell click jumps to Standings; continental tournaments carry their
    // confederation through, global tournaments (Olympics/WC) map to nothing.
    ...
    var footnote = anyPreRated
      ? '<p class="sport-note"><sup>†</sup> Early editions with incomplete game data. Podium shown for completeness; ratings are not available at these editions\' final dates.</p>'
      : "";
  ...
  buildPillsFromOpts("historyWorldPills", HISTORY_WORLD, state.historyTourny, onHistoryPick, ["historyContinentalPills"]);
  buildPillsFromOpts("historyContinentalPills", HISTORY_CONTINENTAL, state.historyTourny, onHistoryPick, ["historyWorldPills"]);
