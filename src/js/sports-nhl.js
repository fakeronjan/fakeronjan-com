(function () {
  ...
  var cupOddsTitle = "Probability of winning the Stanley Cup, from simulating the rest of the regular season and the playoffs with current ratings. League-wide probabilities sum to 100%.";
  ...
  // Knockout odds, ported from MESSI's World Cup grid (same code as sports-nba.js). Data: per-season
  ...
  var poNav = document.getElementById("nhlPoNav");
  ...
  function renderPlayoffOdds() {
    ...
    if (view.results && view.results.length) {
      poStamp.innerHTML = '<span class="wc-md-label">Results · ' + view.date + "</span>" + view.results.map(function (g) {
        var ot = g.ot ? ' <span class="wc-so">(' + g.ot + ")</span>" : "";
        return '<span class="wc-result">' + g.home + " <b>" + g.hp + "-" + g.vp + "</b> " + g.away + ot + "</span>";
      }).join("");
    } else {
