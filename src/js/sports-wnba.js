(function () {
  ...
  // ── formatting helpers, ported 1:1 from LOBO's docs/index.html ────────────
  ...
  // The WNBA used conference-bracketed playoffs only through the 2015 season.
  // From 2016 onward the league reseeded by record, so making the Finals no
  // longer implies winning your conference. Only flag pre-2016 finalists.
  function confBadge(conf, finalsStatus, season) {
    if (!conf) return "";
    if (finalsStatus >= 1 && season != null && season < 2016) {
  ...
  function finishBadge(finalsStatus, cupStatus) {
    var bits = [];
    if (finalsStatus === 2) bits.push('<span class="finish-emoji" title="WNBA Champion">👑</span>');
    else if (finalsStatus === 1) bits.push('<span class="finish-emoji" title="WNBA Runner-Up">🥈</span>');
    if (cupStatus === 2) bits.push('<span class="finish-badge finish-champion" title="Commissioner\'s Cup Champion">Cup 🏆</span>');
    else if (cupStatus === 1) bits.push('<span class="finish-badge finish-runner" title="Commissioner\'s Cup Runner-Up">Cup 🥈</span>');
    return bits.join(" ");
  }
  ...
  // Commissioner's Cup final: a real played game that moves the rating but
  // does NOT count in the W-L record. Tagged so that's self-evident.
  function renderLastMatch(raw, season, isStale, isCup) {
    var display = displayMatch(raw);
    if (!display) return "-";
    var cupTag = isCup
      ? ' <span class="cup-final-tag" title="Commissioner\'s Cup final (counts for ratings, not the W-L record)">Cup Final</span>'
      : "";
    var rc = resultClass(raw, isStale);
    var m = display.match(LAST_MATCH_RE);
    if (!m) return '<span class="' + rc + '">' + display + "</span>" + cupTag;
    ...
    var oppHtml = slug
      ...
      : opponent;
    return '<span class="' + rc + '">' + letter + " " + score + " " + venue + " " + oppHtml + "</span>" + cupTag;
  }
  ...
  document.getElementById("wnbaTabs").addEventListener("click", function (e) {
  ...
  var seasonSelect = document.getElementById("wnbaSeason");
  var dateSelect = document.getElementById("wnbaDate");
  var countEl = document.getElementById("wnbaCount");
  var warmupNote = document.getElementById("wnbaWarmup");
  var standingsTableWrap = document.getElementById("wnbaStandingsTable");
  var dateRangeEl = document.getElementById("wnbaDateRange");
  var refreshedEl = document.getElementById("wnbaRefreshed");

  var titleOddsTitle = "Probability of winning the WNBA Finals, from simulating the rest of the regular season and the playoffs with current ratings. League-wide probabilities sum to 100%.";
  ...
  function renderStandings() {
    ...
    var rows = teams
      .map(function (t) {
        ...
        var lastGameHtml = renderLastMatch(t.last_match, season, isStale, t.last_match_is_cup);
        ...
        return (
          ...
          teamTd +
          '<td class="col-hide-mobile col-conf">' + confBadge(t.conference, t.finals_status, season) + "</td>" +
          ...
          '<td class="col-hide-mobile" style="text-align:center;white-space:nowrap">' + finishBadge(t.finals_status, t.cup_status) + "</td>" +
          "</tr>"
        );
      })
      .join("");
    ...
    standingsTableWrap.innerHTML =
      ...
      '<th class="col-hide-mobile col-od" title="' + titleOddsTitle + '">Title Odds</th><th class="col-last-match">Last Game</th>' +
  ...
  function populateDateSelect() {
    ...
    dateSelect.value = snaps[snaps.length - 1].date;
  }
  ...
  function loadSeason(year) {
    return Promise.all([
      ...
      .then(function (results) {
        ...
        populateDateSelect();
        warmupNote.hidden = Number(year) !== 1997;
        updateDisruptedNote("wnbaDisrupted", [year]);
        var snaps = state.seasonData.snapshots;
  ...
  function renderTeamTable() {
    ...
    var tableRows = rows.slice().reverse().map(function (g) {
      ...
      return (
        "<tr>" +
        '<td class="col-rank linked" data-season-link="' + g.season + '">' + seasonCell + "</td>" +
    ...
    tsTableWrap.innerHTML =
      ...
      '<th class="col-hide-mobile col-od">OFF</th><th class="col-hide-mobile col-od">DEF</th>' +
      '<th class="col-hide-mobile col-od" title="' + titleOddsTitle + '">Title Odds</th><th class="col-hide-mobile col-conf">Conf</th>' +
      '<th class="col-hide-mobile">Honors</th>' +
      "</tr></thead><tbody>" + tableRows + "</tbody></table>";
    attachLinks(tsTableWrap);
  }

  // Hand-rolled SVG rating-over-time chart, ported from LOBO's drawChart().
  function drawChart(rows, seasonFilter) {
    ...
    rows.forEach(function (r, i) {
      ...
      var titleText = (r.finals_status === 2 ? "WNBA Champion (" : "WNBA Runner-Up (") + r.season + ")";
      var x = px(i);
  ...
  function cupInlineBadge(t) {
    if (t.cup_status === 2) return ' <span class="finish-badge finish-champion" title="Commissioner\'s Cup Champion">Cup 🏆</span>';
    if (t.cup_status === 1) return ' <span class="finish-badge finish-runner" title="Commissioner\'s Cup Runner-Up">Cup 🥈</span>';
    return "";
  }
  ...
  function championTeamCell(t, bg, season) {
    ...
    var slug = state.nameToSlug[t.team];
    var nameHtml = t.team + countStr(t) + cupInlineBadge(t);
    var teamTd = slug
      ? '<td class="' + bg + ' team-cell linked" data-team-slug="' + slug + '" data-season="' + season + '">' + nameHtml + "</td>"
      : '<td class="' + bg + ' team-cell">' + nameHtml + "</td>";
    return (
      teamTd +
  ...
  function seriesScoreCell(e) {
    return '<td class="divider-col">' + (e.series_score || "") +
      (e.final_score ? '<div class="sub-line">(' + e.final_score + ")</div>" : "") + "</td>";
  }

  function renderChampions() {
    if (!state.championsData) return;
    var entries = state.championsData.WNBA || [];
    if (!entries.length) {
  ...
  // ── WNBA Finals rankings sub-view ────────────────────────────────────────

  var PRE_FINALS_MODES = ["best-matchup", "worst-matchup", "closest", "blowout", "upset"];
  var OFF_MODES = ["best-off-champs", "worst-off-champs"];
  var DEF_MODES = ["best-def-champs", "worst-def-champs"];

  function preFinalsView(t) { return Object.assign({}, t, { rating: t.rating_pre, rank: t.rank_pre, playoff_record: t.playoff_record_pre }); }
  ...
  function renderRankingTable(rows, mode, metricLabel) {
    ...
    var isPreFinals = showCombined || showDiff;
    ...
    var preTag = isPreFinals ? '<div class="sub-line">(pre-Finals)</div>' : "";
  ...
  function renderRankings(rankType) {
    ...
    var isPreFinals = PRE_FINALS_MODES.indexOf(rankType) !== -1;
    var metricLabel = isOff ? "Offense" : isDef ? "Defense" : "Rating";

    var entries = (state.championsData.WNBA || []).filter(function (e) {
    ...
    var rows = entries.map(function (e) {
      var c, r;
      if (isPreFinals) { c = preFinalsView(e.champion); r = preFinalsView(e.runner_up); }
      ...
      return { season: e.season, series: e.series_score || "", score: e.final_score || "", champ: c, loser: r, quality: hi + 2 * lo, diff: c.rating - r.rating };
    });
    ...
    switch (rankType) {
      case "best-champs":
        ...
        note = "Top 10 champions by end-of-playoffs rating. The strongest teams ever to win a WNBA Finals.";
        mode = "team-vs-opp"; break;
      case "worst-champs":
        sorted = rows.slice().sort(function (a, b) { return a.champ.rating - b.champ.rating; }).slice(0, 10);
        note = "Bottom 10 champions by end-of-playoffs rating. The weakest teams ever to win a WNBA Finals.";
        mode = "team-vs-opp"; break;
      case "best-losers":
        sorted = rows.slice().sort(function (a, b) { return b.loser.rating - a.loser.rating; }).slice(0, 10);
        note = "Top 10 runner-ups by end-of-playoffs rating. The strongest teams ever to lose a WNBA Finals.";
        mode = "loser-vs-champ"; break;
      case "worst-losers":
        sorted = rows.slice().sort(function (a, b) { return a.loser.rating - b.loser.rating; }).slice(0, 10);
        note = "Bottom 10 runner-ups by end-of-playoffs rating. The weakest teams ever to lose a WNBA Finals.";
        mode = "loser-vs-champ"; break;
      case "best-off-champs":
        sorted = rows.slice().sort(function (a, b) { return b.champ.rating - a.champ.rating; }).slice(0, 10);
        note = "Top 10 champions by Offense rating. The most explosive offenses ever to win a WNBA Finals.";
        mode = "team-vs-opp"; break;
      ...
      case "best-def-champs":
        sorted = rows.slice().sort(function (a, b) { return b.champ.rating - a.champ.rating; }).slice(0, 10);
        note = "Top 10 champions by Defense rating. The most suffocating defenses ever to win a WNBA Finals.";
        mode = "team-vs-opp"; break;
      ...
      case "best-matchup":
        ...
        note = "Top 10 by pre-Finals matchup quality. Formula: max(Rating) + 2 × min(Rating), which weights the weaker team double to reward balanced strength over one-sided star power.";
        mode = "matchup"; break;
      case "worst-matchup":
        sorted = rows.slice().sort(function (a, b) { return a.quality - b.quality; }).slice(0, 10);
        note = "Bottom 10 by pre-Finals matchup quality. Formula: max(Rating) + 2 × min(Rating), which weights the weaker team double to penalise lopsided matchups over evenly-mediocre ones.";
        mode = "matchup"; break;
      case "closest":
        sorted = rows.slice().sort(function (a, b) { return Math.abs(a.diff) - Math.abs(b.diff); }).slice(0, 10);
        note = "Top 10 by smallest pre-Finals rating gap. Signed diff is from the champion's POV (negative = upset).";
        mode = "spread"; break;
      case "blowout":
        sorted = rows.slice().sort(function (a, b) { return Math.abs(b.diff) - Math.abs(a.diff); }).slice(0, 10);
        note = "Top 10 by largest pre-Finals rating gap. The biggest predicted mismatches, regardless of who actually won.";
        mode = "spread"; break;
      case "upset":
        sorted = rows.filter(function (r) { return r.diff < 0; }).sort(function (a, b) { return a.diff - b.diff; });
        note = sorted.length + " WNBA Finals where the model picked the loser by pre-Finals rating. Sorted from biggest upset down.";
        mode = "spread"; break;
      default:
  ...
  function renderRepeatChamps() {
    var entries = (state.championsData.WNBA || []).slice().sort(function (a, b) { return a.season - b.season; });
    var runs = [];
    var current = [];
  ...
  function renderRematches() {
    ...
    historyRankNote.textContent = groups.length + " WNBA Finals rematch groups (" + totalMeetings + " total meetings). A group continues as long as the same two teams meet again within " + REMATCH_GAP_THRESHOLD + " years.";
    if (!groups.length) {
  ...
  function renderBiggestLeap() {
    var rows = (state.championsData.WNBA || [])
    ...
    var body = rows.map(function (r, i) {
      var c = r.c;
      var slug = state.nameToSlug[c.team];
      var nameHtml = c.team + countStr(c) + cupInlineBadge(c);
      var teamTd = slug
        ? '<td class="team-cell linked" data-team-slug="' + slug + '" data-season="' + r.e.season + '">' + nameHtml + "</td>"
  ...
  function renderTitleOddsList(direction) {
    ...
    historyRankTableWrap.innerHTML =
      '<table class="sport-table sport-table-narrow"><thead><tr>' +
      '<th class="col-rank">#</th><th class="col-rank">Season</th><th>Champion</th><th class="col-hide-mobile col-record">W-L</th>' +
      '<th class="col-hide-mobile col-rank">Odds rank</th><th class="sort-col col-od">Title odds</th>' +
      "</tr></thead><tbody>" + body + "</tbody></table>";
    attachLinks(historyRankTableWrap);
  }
  ...
  function renderGoat() {
    ...
    var rows = teams.map(function (t) {
      ...
      return (
        ...
        teamTd +
        '<td class="col-hide-mobile col-conf">' + confBadge(t.conference, t.finals_status, t.season) + "</td>" +
        '<td class="col-hide-mobile col-record">' + fmtRecordSmart(t.regular_record, t.playoff_record) + "</td>" +
        metricCells +
        '<td class="col-hide-mobile" style="white-space:nowrap">' + finishBadge(t.finals_status, t.cup_status) + "</td>" +
        "</tr>"
      );
    }).join("");
  ...
  buildPills("goatConfPills", state.goatConf, function (v) { state.goatConf = v; renderGoat(); }, [
    { value: "ALL", label: "All" }, { value: "East", label: "East" }, { value: "West", label: "West" },
  ]);
  ...
  function renderPlayoffOdds() {
    ...
    var nR = short.length;
    poTitle.textContent = d.season + " WNBA Playoffs 🏆 Win Probability";
    ...
    if (view.results && view.results.length) {
      poStamp.innerHTML = '<span class="wc-md-label">Results · ' + view.date + "</span>" + view.results.map(function (g) {
        return '<span class="wc-result">' + g.home + " <b>" + g.hp + "-" + g.vp + "</b> " + g.away + "</span>";
      }).join("");
    } else {
  ...
  Promise.all([
    ...
    buildPills("wnbaConfPills", state.standingsConf, function (v) { state.standingsConf = v; renderStandings(); }, [
      { value: "ALL", label: "All" }, { value: "East", label: "East" }, { value: "West", label: "West" },
    ]);
